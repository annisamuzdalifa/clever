...
class ExchangeFlow(
    ...
    override fun call(): String {
        ...
        val amountReceived = lpoolSession.sendAndReceive<Pair<List<StateAndRef<FungibleToken>>, List<FungibleToken>>>(
            mapOf(Pair("amount", amount), Pair("currencyDest", currencyDest), Pair("currencyOrigin", currencySource)))
            .unwrap { it }
        ...
        val outputPool = PoolTransactionState(
            ...
            participants = listOf(ourIdentity, lPool)
        )
        val command = Command(PoolTransactionContract.Commands.Exchange(), outputPool.participants.map { it.owningKey }+lPool.owningKey)
...
@InitiatedBy(ExchangeFlow::class)
class ExchangeResponderFlow(val otherSession: FlowSession) : FlowLogic<SignedTransaction>() {

    @Suspendable
    override fun call(): SignedTransaction {
        // for exchange this flow always run by LPManager
        ...
        val inputsAndOutputs = LocalTokenSelector(serviceHub).generateMove(
            partiesAndAmounts = listOf(Pair(otherSession.counterparty, amount(totalReceived, TokenType(currencyDest, 2)))),
            changeHolder = ourIdentity,
            holdingKey = ourIdentity.owningKey
        )
